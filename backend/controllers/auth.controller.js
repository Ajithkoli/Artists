...
exports.register = async (req, res, next) => {
  try {
    ...
    if (existingUser)
      return next(
        new ErrorHandler("An account with this email already exists.", 409)
      );
    ...
    if (type === "artist") {
      if (!specialization)
        return next(
          new ErrorHandler("Specialization is required for artists.", 400)
        );
...
exports.login = async (req, res, next) => {
  try {
    ...
    if (user.role === "artist" && user.status !== "approved") {
      return next(
        new ErrorHandler("Your account is still pending approval.", 403)
      );
    }
